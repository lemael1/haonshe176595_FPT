...
public class SumServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String result_raw = request.getParameter("a");
        if (result_raw.equals("RESET")) {
            request.getRequestDispatcher("index.html").forward(request, response);
        } else if (result_raw.equals("SUM")) {
            String number_raw = request.getParameter("number");
            try {
                int num = Integer.parseInt(number_raw);
                if (num < 1) {
                    out.println("You input wrong");
                } else {
                    int Sum;
                    Sum = PrimeTotal(num);
                    out.println(Sum);
